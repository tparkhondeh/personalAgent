...
type ContextPreference = { workdayStartsAt: string; workdayEndsAt: string; workingDays: string; defaultReminderMins: number; quietHoursStartsAt: string; quietHoursEndsAt: string; planningProfile: string | null } | null;
...
export function formatAgentContext({ preference, tasks, meetings }: { preference: ContextPreference; tasks: ContextTask[]; meetings: ContextMeeting[] }) {
  const planning = preference ? {
    ...
    defaultReminderMins: preference.defaultReminderMins,
