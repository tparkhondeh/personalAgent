...
export type Plan = {
  ...
  channels: ("IN_APP" | "PUSH" | "NATIVE" | "ALARM")[];
  repeatCount: number;
...
export function extractPersianTitle(message:string) {
  ...
  text=text.replace(clock,(match,offset:number,source:string)=>/(?:خرید|تعمیر|فروش|تعویض)\s*$/.test(source.slice(0,offset))?match:" ")
  ...
  if(call)text="تماس با "+call[1].replace(/\s+بابت\s+/," درباره ");
  const prepare=text.match(/^(.+?)\s+(?:آماده کنم|آماده کن|تهیه کنم|تهیه کن)$/);
  if(prepare)text="آماده‌سازی "+prepare[1];
  const buy=text.match(/^(.+?)\s+(?:بخرم|بخر)$/);if(buy)text="خرید "+buy[1];
  const pay=text.match(/^(.+?)\s+(?:پرداخت کنم|پرداخت کن)$/);if(pay)text="پرداخت "+pay[1];
  text=text.replace(/SUBJECTTOKEN(\d+)END/g,(_,index)=>protectedSubjects[Number(index)]);
  if(/^(?:این|اینو|آن|اونو|همون|اون کار|این کار|یه کار|یادم بنداز)$/.test(text))return "";
...
export function planInstant(date: string, time: string, timezone: string): Date | null {
...
function shiftDay(day: string, count: number) {
...
export function plannedReminderTimes(plan: Plan, now = new Date()) {
  if(plan.operation!=="CREATE" && plan.operation!=="UPDATE")return [];
  return planOccurrences(plan).flatMap(occurrence=>{
    if(!occurrence.instant)return [];
    ...
    return plan.reminderOffsets.filter(m=>reference-m*60000>now.getTime()).map(offset=>{
      ...
      if(quiet){
        ...
        if(end)scheduled=end.getTime();
        else { // DST can skip the configured wall time. Find the first valid minute outside quiet hours.
          for(let i=0;i<1440;i++){scheduled+=60000;const next=dateParts(new Date(scheduled),plan.timezone).time;if(!(overnight?next>=plan.quietStart||next<plan.quietEnd:next>=plan.quietStart&&next<plan.quietEnd))break;}
        }
      }
...
function jalaliDate(year: number, month: number, day: number) {
  if (year < 1300 || year > 1500 || month < 1 || month > 12 || day < 1 || day > 31) return "";
  ...
  for (let i = 0; i < 370; i++) {
    ...
    const value = (key: string) => Number(p.find(x => x.type === key)?.value);
    if (value("year") === year && value("month") === month && value("day") === day) return date.toISOString().slice(0, 10);
  }
  return "";
}
...
export function inspectPlan(plan: Plan, now = new Date(), items: PlanningItem[] = []) {
  ...
  if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(plan.quietStart)||!/^([01]\d|2[0-3]):[0-5]\d$/.test(plan.quietEnd))questions.push("ساعات سکوت معتبر نیست.");
  if(plan.reminderOffsets.length>8||plan.reminderOffsets.some(m=>!Number.isInteger(m)||m<0||m>10080))questions.push("زمان یادآوری معتبر نیست.");
  if(plan.title.length>180)questions.push("عنوان را کوتاه‌تر کن.");
  if (!plan.title.trim()) questions.push("عنوان کار یا جلسه چیست؟");
  if (plan.operation !== "CREATE" && !plan.targetId) questions.push("کدام مورد را تغییر بدهم؟ از فهرست انتخاب کن.");
  const instant = planInstant(plan.date, plan.time, plan.timezone);
  ...
  if (active) {
    ...
    if (plan.recurrence !== "NONE" && plan.operation !== "CREATE") questions.push("ویرایش گروهی تکرار مبهم است؛ تکرار را ندارد انتخاب کن تا فقط همین نوبت تغییر کند.");
  ...
  if (plan.channels.includes("NATIVE")) warnings.push("Notification پس از همگام‌سازی اپ اندروید با همین حساب و اجازه دستگاه تنظیم می‌شود.");
...
export function planPersian(message: string, context: PlanningContext = {}) {
  ...
  if(context.previous && /^(?:نه[،\s]+)?(?:عنوان|تیتر|اسمش|نامش)(?:ش)?\s*(?:را|رو)?/.test(original.trim())) {
    ...
    return {plan,...checked,candidates:[] as PlanningItem[],reply:checked.questions[0]??"عنوان همین پیشنهاد اصلاح شد؛ بررسی و ثبت کن."};
  }
  ...
  if (/فوری/.test(text)) plan.priority = "URGENT";
  else if (/مهم/.test(text)) plan.priority = "IMPORTANT";
  else if (/عادی/.test(text)) plan.priority = "NORMAL";
  const today = dateParts(now, timezone).date;
  if (/پس فردا|پسفردا/.test(text)) plan.date = shiftDay(today, 2);
  else if (/فردا/.test(text)) plan.date = shiftDay(today, 1);
  else if (/امروز/.test(text)) plan.date = today;
  const explicit = text.match(/\b(1[34]\d{2}|20\d{2})[/-](\d{1,2})[/-](\d{1,2})\b/);
  if (explicit) plan.date = Number(explicit[1]) < 1700 ? jalaliDate(...explicit.slice(1).map(Number) as [number, number, number]) : `${explicit[1]}-${explicit[2].padStart(2, "0")}-${explicit[3].padStart(2, "0")}`;
  ...
  if (/هر هفته|هفتگی/.test(text)) plan.recurrence = "WEEKLY";
  else if (/هر روز|روزانه/.test(text)) plan.recurrence = "DAILY";
  if (/بدون تکرار/.test(text)) plan.recurrence = "NONE";
  const occurrences=text.match(/(\d+)\s*نوبت/);if(occurrences)plan.occurrenceCount=Number(occurrences[1]);
  const offsets = [...text.matchAll(/(\d+)\s*(روز|ساعت|دقیقه)\s*قبل/g)].map(m => Number(m[1]) * (m[2] === "روز" ? 1440 : m[2] === "ساعت" ? 60 : 1));
  if (offsets.length) plan.reminderOffsets = [...new Set(/هم|اضافه/.test(text) ? [...plan.reminderOffsets, ...offsets] : offsets)].sort((a,b) => b-a);
  else if (/یادم|یادآور|یادآوری/.test(text) && !previous) { plan.reminderOffsets = context.offsets ?? [1440,180,60]; plan.defaults.push("یادآوری‌ها از تنظیمات پیش‌فرض"); }
  if (/بدون یادآوری/.test(text)) { plan.reminderOffsets = []; plan.channels = []; }
  if (/آلارم|الارم/.test(text) && !/بدون آلارم|بدون الارم/.test(text)) plan.channels = [...new Set([...plan.channels, "ALARM" as const])];
  if (/اعلان گوشی/.test(text)) plan.channels = [...new Set([...plan.channels, "NATIVE" as const])];
  if (/پوش|push/i.test(text)) plan.channels = [...new Set([...plan.channels, "PUSH" as const])];
  if (/بدون آلارم|بدون الارم/.test(text)) plan.channels = plan.channels.filter(c => c !== "ALARM");
  if (/هشدار فوری|تشدید/.test(text)) plan.escalation = true;
  ...
  if (plan.operation !== "CREATE" && !plan.targetId) {
    ...
    if (candidates.length === 1) {
      const item = candidates[0]; plan.targetId = item.id!; plan.targetUpdatedAt = item.updatedAt ?? null;
      plan.title = item.title; plan.entity = item.entity ?? "TASK";
      if (item.dueAt || item.startsAt) { const parts=dateParts(new Date((item.dueAt || item.startsAt)!),timezone);if(!plan.date)plan.date=parts.date;if(!plan.time && !/عصر|صبح|شب/.test(command))plan.time=parts.time; }
      if(!/شخصی|شرکت|کاری/.test(text) && (item.category==="PERSONAL"||item.category==="WORK"))plan.category=item.category;
      if(!/فوری|مهم|عادی/.test(text) && (item.priority==="NORMAL"||item.priority==="IMPORTANT"||item.priority==="URGENT"))plan.priority=item.priority;
      if(item.alertPolicy){try{const policy=JSON.parse(item.alertPolicy);if(!/یادم|یادآور|یادآوری|اعلان|آلارم|الارم/.test(text)){plan.reminderOffsets=policy.reminderOffsets??[];plan.channels=policy.channels??["IN_APP"];}plan.escalation=policy.escalation??false;plan.repeatCount=policy.repeatCount??1;plan.repeatMinutes=policy.repeatMinutes??15;plan.quietStart=policy.quietStart??plan.quietStart;plan.quietEnd=policy.quietEnd??plan.quietEnd;}catch{}}
  ...
  if (explicit && !plan.date) checked.questions.push("تاریخ شمسی معتبر نیست.");
