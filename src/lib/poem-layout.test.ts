...
describe('readable two-column poem geometry',()=>{
  ...
  it('preserves readable text and natural wrapping on narrow screens',()=>{const g=poemGeometry(240,[250,260,150,190]);expect(g.fontSize).toBe(13);expect(g.gap).toBe(8);expect(g.wrap).toBe(true);});
  it('respects enlarged text instead of scaling it back down',()=>{expect(poemGeometry(320,[380,320,300,400],32).fontSize).toBe(26);});
