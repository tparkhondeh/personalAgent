...
async function loadSpeechLibrary(signal:AbortSignal):Promise<SpeechLibrary> {
  ...
  signal.throwIfAborted();if(!scope.Vosk)throw new Error('engine');return scope.Vosk;
}
  ...
  return {
    ...
    async transcribe(clip:Blob,progress:(message:string)=>void):Promise<string> {
      ...
      try {
        ...
        const library=await loadSpeechLibrary(signal);signal.throwIfAborted();
        ...
        signal.throwIfAborted();progress('در حال تبدیل صدا؛ چیزی هنوز ثبت نشده است…');
        ...
        signal.removeEventListener('abort',terminate);recognizer?.remove();model?.terminate();if(active===controller)active=null;
      }
    },
  };
}
