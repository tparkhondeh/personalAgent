...
export function PersianDateField({value,defaultValue="",onChange,name,required,label="تاریخ شمسی"}:FieldProps){
  ...
  function typeDate(raw:string){
    const text=inputDigits(raw.trim());
    // Keep partial digits until both month and day are complete; formatting a
    // one-digit day here inserts a zero before the user's next keystroke.
    change(/^\d{4}[/-]\d{2}[/-]\d{2}$/.test(text)?parsePersianInput(text):text?`invalid:${text}`:"");
  }
  function finishDate(){if(current.startsWith("invalid:"))change(parsePersianInput(dateInputValue(current)));}
  ...
  return <span className="date-time-control"><input type="hidden" name={name} value={current}/><input aria-label={label} dir="ltr" inputMode="numeric" placeholder="۱۴۰۵/۰۶/۱۵" required={required} pattern="[۰-۹0-9]{4}/[۰-۹0-9]{2}/[۰-۹0-9]{2}" value={faDigits(dateInputValue(current))} onChange={e=>typeDate(e.target.value)} onBlur={finishDate} aria-invalid={current.startsWith("invalid:")}/>
