  ...
  return <section className="assistant-panel" aria-label="گفتگو با tia">
    ...
    {draft && p && summary && <section ref={review} className="agent-review compact-review" aria-label="پیش‌نمایش تأیید"><h2>بررسی پیش از {operations[p.operation]}</h2>
      ...
      <fieldset disabled={!edit || pending}><div className="agent-grid">
        ...
        {p.escalation && <><label>تعداد هشدار پس از موعد<input type="number" min={0} max={6} value={p.repeatCount} onChange={e=>change("repeatCount",Number(e.target.value))}/></label>
        {p.repeatCount>0 && <label>فاصله پیگیری هشدار (دقیقه)<input type="number" min={10} max={1440} value={p.repeatMinutes} onChange={e=>change("repeatMinutes",Number(e.target.value))}/></label>}</>}
