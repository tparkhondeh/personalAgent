...
export function AgentAssistant({ onAdd, onChanged }: { onAdd: () => void; onChanged: () => Promise<void> }) {
  ...
  async function send(event:FormEvent) {
    event.preventDefault();if(!input.trim()||pending)return;setPending(true);setStatus("");
    try {
  ...
  async function act(action:"edit"|"confirm"|"cancel") {
    if(!draft||pending)return;setPending(true);setStatus("");
    try {
  ...
  const format=(value:string)=>new Intl.DateTimeFormat("fa-IR",{timeZone:p?.timezone??"Asia/Tehran",dateStyle:"medium",timeStyle:"short"}).format(new Date(value));
  return <section className="assistant-panel" aria-label="گفتگو با همراه">
    ...
    {draft && p && <section className="agent-review" aria-label="پیش‌نمایش تأیید"><h2>بررسی پیش از {operations[p.operation]}</h2><p>نسخه {draft.revision} — هنوز اجرا نشده</p>
      <fieldset disabled={!edit || pending}><div className="agent-grid">
        <label>عنوان<input value={p.title} maxLength={180} onChange={e=>change("title",e.target.value)} /></label>
        <label>عملیات<select value={p.operation} onChange={e=>change("operation",e.target.value as Plan["operation"])}>{Object.entries(operations).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
        <label>نوع<select value={p.entity} onChange={e=>change("entity",e.target.value as Plan["entity"])}><option value="TASK">کار</option><option value="MEETING">جلسه</option></select></label>
        <label>دسته<select value={p.category} onChange={e=>change("category",e.target.value as Plan["category"])}><option value="PERSONAL">شخصی</option><option value="WORK">شرکتی</option></select></label>
        ...
        <label>تاریخ (ورودی میلادی)<input type="date" value={p.date} onChange={e=>change("date",e.target.value)} /></label>
        <label>ساعت ۲۴ساعته<input type="time" value={p.time} onChange={e=>change("time",e.target.value)} /></label>
        <label>منطقه زمانی<input value={p.timezone} onChange={e=>change("timezone",e.target.value)} /></label>
        {p.entity==="MEETING" && <label>مدت جلسه، دقیقه<input type="number" min={5} max={1440} placeholder="نامشخص" value={p.durationMinutes??""} onChange={e=>change("durationMinutes",e.target.value?Number(e.target.value):null)} /></label>}
        ...
        <label>شروع سکوت<input type="time" value={p.quietStart} onChange={e=>change("quietStart",e.target.value)}/></label>
        <label>پایان سکوت<input type="time" value={p.quietEnd} onChange={e=>change("quietEnd",e.target.value)}/></label>
      ...
      {!edit && <>{draft.preview.instant && <p>زمان دقیق: {format(draft.preview.instant)} — {p.timezone}</p>}<p>{p.defaults.join("؛ ")}</p>{draft.preview.questions.map((q,i)=><p className="agent-question" key={i}>{q}</p>)}{draft.preview.warnings.map((w,i)=><p key={i}>{w}</p>)}<ul className="agent-schedule">{draft.preview.schedule.map((s,i)=><li key={i}>{channels[s.channel as keyof typeof channels]}: {format(s.scheduledFor)}</li>)}</ul></>}
      <div className="agent-actions">{edit?<button disabled={pending} onClick={()=>void act("edit")}>ذخیره و نمایش پیش‌نمایش تازه</button>:<><button disabled={pending || Boolean(draft.preview.questions.length)} onClick={()=>void act("confirm")}>تأیید و اجرا</button><button disabled={pending} onClick={()=>setEdit(structuredClone(draft.plan))}>ویرایش</button></>}<button disabled={pending} onClick={()=>void act("cancel")}>انصراف</button></div>
    </section>}
