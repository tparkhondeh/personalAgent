...
const overviewSource = (await Promise.all(['dashboard-overview','poem-layout','personal-poem','startup-navigation'].map(name=>readFile(path.join(projectRoot, `src/lib/${name}.ts`), 'utf8')))).join('\n');
...
const poemScript = `window.HamrahOverview=(()=>{${overviewJs}\nreturn {selectDashboardScope,selectDashboardItems,summarizeDashboardItems,createPoemNavigator,dailyPoemIndex,tehranDayKey,observePoemLayout,mountPersonalPoemEditor,initialDashboardView};})();\nwindow.HamrahPoems = ${JSON.stringify(poems.selections.map(({ lines }) => lines)).replaceAll("<", "\\u003c")};\n`;
