...
const environment = { buildType: adb("shell", "getprop", "ro.build.type"), debuggable: adb("shell", "getprop", "ro.debuggable"), fingerprint: adb("shell", "getprop", "ro.build.fingerprint") };
writeFileSync(`${output}/release-system-image.json`, JSON.stringify(environment));
assert.equal(environment.buildType, "user", "Cold release security requires a production-like Android image");
assert.equal(environment.debuggable, "0", "Do not accept OS-forced debugging as an APK property");
...
writeFileSync(`${output}/release-debug-sockets.txt`, sockets);
assert(!sockets.split(/\r?\n/).some(line => line.trim().endsWith(`@webview_devtools_remote_${pid}`)), "Cold release must not expose WebView debugging");
