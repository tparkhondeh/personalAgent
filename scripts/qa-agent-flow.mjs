...
assert.equal((await call("/api/meetings")).find(m=>m.id===meetingCreated.entityId).priority,meeting.draft.plan.priority);
const updateSeed=await call("/api/agent",{message:`${future} ساعت 17 جلسه اولویت بساز`,localOnly:true});
const updateReview=await call(`/api/agent/drafts/${updateSeed.draft.id}`,{action:"edit",revision:1,plan:{...updateSeed.draft.plan,operation:"UPDATE",targetId:meetingCreated.entityId,priority:"URGENT"}});
assert.deepEqual(updateReview.preview.questions,[]);
const updateResult=await call(`/api/agent/drafts/${updateSeed.draft.id}`,{action:"confirm",revision:2,confirmed:true});
assert.equal(updateResult.entityId,meetingCreated.entityId);
assert.equal((await call("/api/meetings")).find(m=>m.id===meetingCreated.entityId).priority,"URGENT");
const meetingSeriesSeed=await call("/api/agent",{message:`${future} ساعت 17 جلسه دوره‌ای بساز`,localOnly:true});
const meetingSeriesReview=await call(`/api/agent/drafts/${meetingSeriesSeed.draft.id}`,{action:"edit",revision:1,plan:{...meetingSeriesSeed.draft.plan,priority:"NORMAL",recurrence:"WEEKLY",occurrenceCount:3}});
assert.deepEqual(meetingSeriesReview.preview.questions,[]);
const meetingSeries=await call(`/api/agent/drafts/${meetingSeriesSeed.draft.id}`,{action:"confirm",revision:2,confirmed:true});
assert.equal(meetingSeries.entityIds.length,3);
const savedMeetings=await call("/api/meetings");
for(const id of meetingSeries.entityIds) assert.equal(savedMeetings.find(m=>m.id===id).priority,"NORMAL");
