...
### درخواست تازه: ساده‌سازی تأیید — در حال کنترل انتشار

اصلاحات ساعت ۲۴ساعته، تقویم شمسی، عنوان، فیلدهای یکپارچه و حذف ساعات سکوت از تنظیمات تازه در کد انجام شده است. پشتیبان و نتیجه آزمون‌ها در [گزارش این مرحله](docs/SIMPLIFIED_APPROVAL_2026-09-06.md) ثبت شده‌اند. نسخه ۲۲ زیر هنوز مرجع انتشار قبلی است؛ APK تازه تا موفقیت آزمون سه Android منتشرشده محسوب نمی‌شود.

بازبینی تکمیلی: ۱۶۶ تست و اصلاح پیشوند «یادم بنداز» در موتور مشترک انجام شد. کاندید ۲۵ در تلاش دوم Android 13 نیز موفق شد (همان APK؛ شکست اول ناشی از restart سرویس فونت سیستم ثبت شده است). کاندید بعدی باید این اصلاح تکمیلی عنوان را هم در APK و Staging داشته باشد؛ گزارش نهایی بعد از آزمون فایل تحویلی جایگزین این بخش می‌شود.

مرحله جاری: کاندید ۲۶ با Commit `07f98c06e24b877b8934763a5f94942be76add4d` ساخته شده؛ Staging با Backup `pre-release-20260906T070604Z` به همین Commit ارتقا یافت و Health و ۳۵ کنترل HTTP موفق‌اند. Android 13/16 موفق و Android 14 پس از رخداد مستند restart فونت سیستم در حال آزمون مجدد همان فایل است؛ انتشار نهایی این مرحله هنوز تأیید نشده است.

پس از تکرار همان مرگ سرویس گوگل در Android 14، مسیر تست همان Artifact با پایدارسازی اولیه شبیه‌ساز اضافه شد؛ آزمون‌ها حذف یا ضعیف نشده‌اند و APK تغییر نکرده است. تا تکمیل این مسیر، نسخه ۲۶ تحویل‌شده محسوب نمی‌شود.
