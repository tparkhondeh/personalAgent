...
بررسی تازه سرور: Staging/current/API برابر `94adb4b`؛ HTTPS8443 از Windows هنوز handshake failure. worker عمومی هر دو پورت443/8443 با hash نسخه قدیمی Production و cache14400 می‌آید، درحالی‌که origin3010 فایل جدید/no-store می‌دهد. رفع این سیاست مشترک/انتقال443 نیازمند تأیید محدود Production است. APK عمومی هنوز40 با SHA تأییدشده قبلی است؛ گواهی debug موجود با40 متفاوت است، پس APK جدید یا تغییر package/endpoint بدون مسیر حفظ اطلاعات انجام نشد. گوشی متصل نیست؛ پذیرش Notification/Alarm/صوت، تنظیم SMTP و پشتیبان برون‌سرور/کلید مجاز باقی‌اند. انتقال/همگام‌سازی خودکار داده آفلاین با حساب هنوز وجود ندارد. نسخه شخصی «نهایی» اعلام نمی‌شود. CI/انتشار کد این مرحله بعد از اجرای آن جدا ثبت خواهد شد.
