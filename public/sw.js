self.addEventListener("install", (event) => {
  event.waitUntil(caches.open("hamrah-shell-v1").then((cache) => cache.addAll(["/", "/manifest.webmanifest", "/icon.svg"])));
  self.skipWaiting();
});

self.addEventListener("activate", (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => key.startsWith("hamrah-shell-") && key !== "hamrah-shell-v1").map((key) => caches.delete(key)))));
  self.clients.claim();
});
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith(fetch(request).then((response) => {
    if (response.ok) caches.open("hamrah-shell-v1").then((cache) => cache.put(request, response.clone()));
    return response;
