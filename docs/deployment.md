...
> سند تاریخی آماده‌سازی اولیه است؛ Commitها و ارقام فضای دیسک پایین وضعیت امروز نیستند. برای تصمیم فعلی فقط [گزارش نسخه شخصی۱۴سپتامبر](PERSONAL_USE_2026-09-14.md) و شواهد تازه استفاده شود. هیچ نسخه دیتابیس آزمایشی یا snapshot قدیمی جای دیتابیس زنده قرار نگیرد.
...
Reverse Proxy و Process/Symlink فقط به وضعیت پیش از همان انتشار، که تازه ثبت و تأیید شده، بازگردند؛ شناسه تاریخی `b50c3b6` مقصد خودکار بازگشت امروز نیست. Snapshot قدیمی روی اطلاعات تازه نوشته نشود. در ناسازگاری، زمان‌بند مربوط متوقف، ابتدا وضعیت فعلی حفظ و بازیابی فقط در نسخه جدا آزمایش شود؛ اصلاح داده زنده به برنامه حفظ نوشته‌های جدید و تأیید مشخص وابسته است. سپس Health Check و Login دوباره آزمایش شوند.
