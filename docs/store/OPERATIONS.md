...
- Unsigned preparation AAB/APK were built on 2026-09-14; their hashes, embedded source checks and non-final staging endpoint are recorded in [the follow-up evidence](OFFLINE_STORAGE_2026-09-14.md). They are not a public release or an upgrade for `.stable40`. After approval and final endpoint/signing selection, rebuild from a clean checkout, verify bundled offline assets, then run `gradlew :app:bundleRelease :app:assembleRelease :app:lintRelease :app:testReleaseUnitTest` and inspect the final signed outputs separately. Existing release Gradle configuration has no production signing setup. No new key was created.
