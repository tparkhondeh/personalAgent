...
| Owner phone | Owner screenshots now show `tia آزمایشی 40`, version1.0.40; installed package/signer/byte identity, phone-only data and actual GPT/notification/Alarm acceptance remain unverified |
...
This is NOT the new Tasks/offline/GPT-completion delivery. Existing signing evidence does not allow a compatible upgrade using the available internal signing key. In the owner's subsequent App Info screenshots, the visible name is `tia آزمایشی 40` and version1.0.40, matching the release label/version; do not request those again. Notification and Alarms/reminders permissions visibly show Allowed, but actual delivery is not established. Screenshots do not establish the installed package, certificate or binary hash. No phone is connected and phone-only records have not been inventoried. The next necessary owner detail is whether records were created signed in, without sign-in, or both. Do not uninstall or clear it, change identity/signing, or present the internal candidate as a safe update. After installed identity/data/signature are confirmed, use matching signing material or a separately approved, tested data-preserving migration. Real phone GPT, notification/Alarm and background behavior still require acceptance. Production promotion and immutable asset-cache refinement are distinct gates. Phone images were not added to Git; this evidence update changes no app code, live settings, spending or data.
