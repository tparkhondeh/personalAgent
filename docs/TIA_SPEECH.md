...
Candidate 36 exposed two genuine Android failures and was not published: WebView 109 (Android 13) has no AbortSignal.any; Android's asset packaging expands `.gz` and removes that extension. Device cancellation now uses a baseline AbortController and a cleared timer, with separate timeout/cancel messages. The model retains gzip bytes under an opaque `.model` extension; both pre-emulator APK hash verification and native instrumentation check its presence/content. Never infer WebView capabilities from Android OS version or assume files in mobile-shell are unchanged inside the final APK. Preserve these regressions and record every candidate failure.
